...
%datos2 = load( 'C:\Users\Usuario\Desktop\TFG\matlab\Scripts\jacobianosYmediaAD_SyMNetexf.mat' );
datos3 = load( 'C:\Users\Usuario\Desktop\TFG\matlab\Scripts\jacobianosYmediaMCI_SyMNetexf.mat' );
...
% SyMNet_ventriculo_AD = cell(length(datos2),1);
% SyMNet_talamo_AD = cell(length(datos2),1);
% SyMNet_hipocampo_AD = cell(length(datos2),1);

SyMNet_ventriculo_MCI = cell(length(datos3),1);
SyMNet_talamo_MCI = cell(length(datos3),1);
SyMNet_hipocampo_MCI = cell(length(datos3),1);

for i = 1:length(datos3)
    ...
    SyMNet_ventriculo_MCI{i} = nii_seleccionado;
end

save('region_ventriculo_MCI_SyMNet.mat', 'SyMNet_ventriculo_MCI');
...
for i = 1:length(datos3)
    ...
    % Guardar el archivo .nii con la región seleccionada
    SyMNet_talamo_MCI{i} = nii_seleccionado;
end

save('region_talamo_MCI_SyMNet.mat', 'SyMNet_talamo_MCI');
...
for i = 1:length(datos3)
    ...
    % Guardar el archivo .nii con la región seleccionada
    SyMNet_hipocampo_MCI{i} = nii_seleccionado;
end

save('region_hipocampo_MCI_SyMNet.mat', 'SyMNet_hipocampo_MCI');