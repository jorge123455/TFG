...
coordenadas = load( 'C:\Users\Usuario\Desktop\TFG\matlab\Scripts\regiones_voxelmorph.mat' );
coordenadas = coordenadas.regiones_voxelmorph;

%datos1 = datos1.jacobianos_CN;
%datos2 = datos2.jacobianos_AD;
datos3 = datos3.jacobianos_MCI;

% voxelmorph_ventriculo_CN = cell(length(datos1),1);
% voxelmorph_talamo_CN = cell(length(datos1),1);
% voxelmorph_hipocampo_CN = cell(length(datos1),1);
...
for i = 1:length(datos3)
    ...
    voxelmorph_ventriculo_MCI{i} = nii_seleccionado;
end

save('region_ventriculo_MCI_volxelmorph.mat', 'voxelmorph_ventriculo_MCI');
...
for i = 1:length(datos3)
    ...
    % Guardar el archivo .nii con la región seleccionada
    voxelmorph_talamo_MCI{i} = nii_seleccionado;
end

save('region_talamo_MCI_volxelmorph.mat', 'voxelmorph_talamo_MCI');
...
for i = 1:length(datos3)
    ...
    % Guardar el archivo .nii con la región seleccionada
    voxelmorph_hipocampo_MCI{i} = nii_seleccionado;
end

save('region_hipocampo_MCI_volxelmorph.mat', 'voxelmorph_hipocampo_MCI');